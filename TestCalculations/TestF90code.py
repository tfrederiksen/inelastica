...
def compare_H(HS1,HS2):
    ...
    if N.sum(N.abs(HS1.lasto-HS2.lasto)) +N.sum(N.abs(HS1.numh-HS2.numh)) \
            +N.sum(N.abs(HS1.listh-HS2.listh)) > tol:
...
def main():
    ...
    file = 'Self-energy-FCC100/ELEC-1x1/ABAB.TSHS'
    HS1=SIO.HS('../TestCalculations/' + file,UseF90helpers=True)
    HS2=SIO.HS('../TestCalculations/' + file.replace('ABAB','ABAB_NEW'),UseF90helpers=True)
    compare_H(HS1,HS2)

    file = 'Self-energy-FCC111/ELEC-1x1/Au3D_BCA.TSHS'
    HS1=SIO.HS('../TestCalculations/' + file,UseF90helpers=True)
    HS2=SIO.HS('../TestCalculations/' + file.replace('BCA','BCA_NEW'),UseF90helpers=True)
    compare_H(HS1,HS2)
