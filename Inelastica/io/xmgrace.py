...
This module provides a `Python`_ interface to `XMGR/GRACE <xmgrace_>`_ 
(get the newest information about `GRACE <xmgrace_>`_ and download the 
latest version at `http://plasma-gate.weizmann.ac.il/Grace/`).

To create a `GRACE <xmgrace_>`_ file from a given data set, the minimal 
script structure will take the following form:
...
The function ``ArrangeGraphs(...)`` is called to arrange the graphs 
nicely on the canvas (side by side).

A more detailed demonstration of this module can be generated by typing 
``python WriteXMGR.py`` in the command promt. This will create a file 
`test.agr`.
