...
class Symmetry(object):
    ...
    def symmetrizeFC(self, FC, FCfirst, FClast, radi=0.0):
        ...
        def applySym(FC):
            ...
            FCn2 = 0*FCn
            for ii in range(NFC):
                tmp = N.tensordot(PL[iU, ii, :, :], FCn[ii, :, :, :], ((1, 1)))
    ...
    def findLattice(self):
        ...
        while i1 < NP-2 and not done:
            ...
            if distance(N.cross(a1, a2)) > self.accuracy: # Non-parallel
                a3 = possible[i3]
        ...
        if not done:
            ...
            print("Symmetry: WARNING Could not find smaller unitcell")
            sys.exit('Symmetry error')
...
def myIntersect(x, y, accuracy):
    ...
    if len(x)*len(y) != 0:
        ...
        if len(z) == 0:
            ...
            return myUnique2(N.array(z), accuracy)
    else:
        return []
